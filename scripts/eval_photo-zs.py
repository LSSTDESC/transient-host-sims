...
chunks = 32#int(os.getenv('SLURM_ARRAY_TASK_COUNT', '32'))

# idx from job array, one core with 128GB memory
batch_size = 25#100#don't change me! (determines memory needed)
# batch_factor = 100
...
# hl_df = hl_df_colors[['redshift', 'u-g', 'g-r', 'r-i', 'i-z', 'z-y', 'r']][:nhost]#, 'u-g_err', 'g-r_err', 'r-i_err', 'i-z_err', 'z-y_err', 'r_err']][:nhost]
...
# print('running external chunk '+str(idx)+' with '+str(len(hl_subset))+' hosts in internal chunks of '+str(batch_size))
# flow_z = flow.posterior(hl_subset,#[['u-g', 'g-r', 'r-i', 'i-z', 'z-y', 'r']], 
#                             column='redshift', grid=zgrid, batch_size=min(batch_size, len(hl_subset)))#, err_samples=10)
