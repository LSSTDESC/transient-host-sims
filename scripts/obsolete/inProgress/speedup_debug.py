...
start = time.time()
#nn = NearestNeighbors(n_neighbors=n_neigh, algorithm='ball_tree', n_jobs=-1).fit(sim_keyparams_norm)
#sim_keyparams_nn = sim_keyparams_norm.copy(order='C')
#sim_keyparams_nn = sim_keyparams_nn.astype(np.float32)
#nn = FaissKNeighbors(n_neighbors=n_neigh).fit(sim_keyparams_nn)
#dist, indices = nn.index.search(sim_keyparams_nn, nn.k)
#nn  = LSHForest(n_estimators=20, n_candidates=200,
#                     n_neighbors=n_neigh).fit(sim_keyparams_norm)
#dist, indices = nn.kneighbors(data_keyparams_norm, return_distance=True)
from nearpy import Engine
from nearpy.hashes import RandomBinaryProjections

np.savez(sim_keyparams_norm, "./sim_keyparams_norm.npz")
print("Saved sim_keyparams_norm")
...
for index in np.arange(len(sim_keyparams_norm)):
    ...
    engine.store_vector(row, 'data_%d' % index)
